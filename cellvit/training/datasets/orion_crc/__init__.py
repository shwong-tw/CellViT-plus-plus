# -*- coding: utf-8 -*-
# ORION-CRC Dataset for Cell Classifier Training
#
# Dataset sources:
#   - Processed tiles: https://zenodo.org/records/15340874
#   - Original WSIs: https://zenodo.org/records/7637988
#
# This dataset class loads preprocessed ORION-CRC data (output of prepare_orion_crc.py)
...
class ORIONCRCDataset(Dataset):
    """ORION-CRC Dataset for CellViT++ Cell Classifier Training.

    Loads preprocessed ORION-CRC patches with cell detection annotations.
    Compatible with the CellViTHeadTrainer training loop.

    Expected directory structure (output of prepare_orion_crc.py):
        dataset_path/
        ├── train/
        │   ├── images/     (*.png, 512x512 RGB H&E patches)
    ...
    Args:
        dataset_path (Union[Path, str]): Path to the preprocessed ORION-CRC dataset
    ...
    def __init__(
        ...
        if not self.image_path.exists():
            raise FileNotFoundError(
                ...
                f"Run prepare_orion_crc.py first to preprocess the dataset."
            )
        ...
        self.images = sorted(
            f for f in self.image_path.glob("*")
    ...
    def cache_dataset(self) -> None:
        ...
        for img_path, annot_path in tqdm.tqdm(
            ...
            desc=f"Caching ORION-CRC {self.split}",
        ):
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, list, list, str]:
        ...
        # Types stored as 1-indexed in JSON (Background=0 excluded),
        # convert to 0-indexed for classifier training
